...
for event, elem in ET.iterparse(xmldata, tag='item', strip_cdata=False, remove_blank_text=True):
	...
	if title is not None:
		...
		if type == 'attachment':
			...
			att = elem.find('wp:attachment_url', namespaces=namespaces);
			att_url = att.text
			shawn_att_url = re.sub(r'http://[\w+/.-]+/', '', att_url)
			print 'ATTACHMENT URL IS ' + att_url
			print 'SHAWN ATTACHMENT IS ' + shawn_att_url
			att.text = shawn_att_url
			elem.find('guid').text = shawn_att_url
